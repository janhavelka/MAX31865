...
|---|---|
| RTD support | Handles 100 Ohm to 1 kOhm platinum RTDs at 0degC, including PT100 to PT1000. [Source: MAX31865 RTD-to-Digital Converter, p. 1] |
...
| Nominal temperature resolution | 0.03125degC nominal temperature resolution; the datasheet notes that this varies due to RTD nonlinearity. [Source: MAX31865 RTD-to-Digital Converter, p. 1] |
| Total accuracy | 0.5degC max total accuracy over all operating conditions, stated as 0.05% of full scale. [Source: MAX31865 RTD-to-Digital Converter, p. 1] |
...
The datasheet states that common PT-RTD nominal resistance values at 0degC are 100 Ohm and 1 kOhm, although other values are available. [Source: MAX31865 RTD-to-Digital Converter, p. 10]

The datasheet lists platinum RTD characteristics as a wide temperature range to over +800degC, excellent accuracy and repeatability, and reasonable linearity. [Source: MAX31865 RTD-to-Digital Converter, p. 10]

The datasheet defines the average slope between 0degC and +100degC as alpha (alpha), and states that two widely used alpha values are 0.00385 and 0.00392, corresponding to the IEC 751 (PT100) and SAMA standards. [Source: MAX31865 RTD-to-Digital Converter, p. 10]
...
where `T` is temperature in degC, `R(T)` is resistance at `T`, and `R0` is resistance at `T = 0degC`. [Source: MAX31865 RTD-to-Digital Converter, p. 10]

IEC 751 coefficient values in the datasheet are: `a = 3.90830 x 10^-3`, `b = -5.77500 x 10^-7`, and `c = -4.18301 x 10^-12` for `-200degC <= T <= 0degC`, with `c = 0` for `0degC <= T <= +850degC`. [Source: MAX31865 RTD-to-Digital Converter, p. 10]
...
The design solution states that the PT100 overtemperature resistance range is 18.52 Ohm at -200degC to 390 Ohm at +850degC. [Source: Achieve High-Accuracy Temperature Measurement in Your Precision Designs, p. 2]
...
The design solution states that the MAX31865 with an RTD provides 0.03125degC resolution across a -200degC to +850degC temperature range with 0.5degC accuracy; this statement is preserved as supplemental wording, and the range/IC-operating-range ambiguity is recorded in `08_variant_differences_and_open_questions.md`. [Source: Achieve High-Accuracy Temperature Measurement in Your Precision Designs, p. 3]
...
For the range -100degC to +100degC, the datasheet gives this approximate temperature relationship:

```text
Temperature (degC) ~= (ADC code / 32) - 256
```

The same paragraph states that this approximation gives 0degC error at 0degC, -1.75degC error at -100degC, and -1.4degC error at +100degC, assuming an IEC 751 RTD and RREF equal to four times the 0degC RTD resistance. [Source: MAX31865 RTD-to-Digital Converter, p. 11]
...
A 2-wire connection can give acceptable results when the RTD is close to the MAX31865. For PT100, the datasheet states that 0.4 Ohm series resistance causes about 1degC error, so cable-resistance error can become excessive as cable length increases. [Source: MAX31865 RTD-to-Digital Converter, p. 10]

The design solution states that reasonable RCABLE values can be as high as 50 Ohm and that a 2-wire RTD with 10 Ohm cable resistance creates approximately 25degC error across the entire temperature range. [Source: Achieve High-Accuracy Temperature Measurement in Your Precision Designs, p. 2]
