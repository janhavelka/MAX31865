...
def main() -> int:
    ...
    api_smoke_main = ROOT / "examples" / "02_api_smoke" / "main.cpp"

    ensure_exists(common_dir, "common example directory")
    ensure_exists(bringup_main, "bringup CLI example")
    ensure_exists(api_smoke_main, "API smoke example")
    ...
    for token in MANDATORY_DIAGNOSTIC_TOKENS:
        if re.search(rf"\b{re.escape(token)}\b", text) is None:
            fail(f"mandatory diagnostic token '{token}' missing in {bringup_main.as_posix()}")
